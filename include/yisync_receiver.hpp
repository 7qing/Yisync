...
struct ChunkCheckpointResult {
  std::uint64_t files = 0;
  std::uint64_t chunks = 0;
  std::uint64_t bytes = 0;
};

struct ChunkCheckpointTask {
...
struct ChunkCheckpointBatch {
  ChunkCheckpointResult result;
  std::vector<ChunkCheckpointTask> tasks;
};
...
struct ChunkCommitTask {
  ...
  std::filesystem::path meta_path;
  ChunkCheckpointTask checkpoint;
...
class ChunkedReceiverStream {
 public:
  ...
  void refresh_committed_from_disk();
  std::uint64_t pending_checkpoint_bytes() const noexcept;
  ChunkCheckpointBatch checkpoint();
  static void write_checkpoint_task(const ChunkCheckpointTask& task);
  std::optional<Nack> prepare_commit(const FileCommit& commit, ChunkCommitTask& task);
  void abort_commit(std::uint64_t order_seq) noexcept;
  void finish_commit(const ChunkCommitResult& result);
  static ChunkCommitResult write_commit_task(const ChunkCommitTask& task);
  std::vector<MissingChunkRange> missing_ranges(std::uint64_t order_seq,
